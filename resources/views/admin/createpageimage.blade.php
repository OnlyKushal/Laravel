                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="login-form">
                                    <form action="{{route('createpagesimagepost')}}" method="post" enctype="multipart/form-data">
                                            @csrf
                                        <div class="form-group">
                                            <label>Page name</label>
                                            <select class="form-control au-input au-input--full" name="pagename">
                                                @foreach ($data as $item)
                                                    @if ($item->submenus==1){
                                                        @foreach ($subpage as $i)
                                                            @if ($item->name == $i->mainpage)
                                                                <option value ="{{$i->slug}}">{{$i->name}}</option>
                                                            @endif
                                                        @endforeach
                                                    }@else{<option value ="{{$item->slug}}">{{$item->name}}</option>}
                                                    @endif
                                                @endforeach
                                            </select>
                                        </div>
